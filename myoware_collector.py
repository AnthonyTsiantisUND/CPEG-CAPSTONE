...
#                        session-only plot + PNG snapshot, plot resets on each recording
# Keys:
...
class Collector:
    def __init__(self):
        ...
        self.max_cols = 6
        self.cols = [[] for _ in range(self.max_cols)]
        self.col_count = 0
        self.col_idx = 0
        ...
        self.rows = []           # dicts to be written to CSV
        self.session_start_idx = [0 for _ in range(self.max_cols)]  # where current session starts in each column
        self.session_started_at = None  # timestamp when SPACE pressed (for optional time axis later)
    ...
    def _reset_session_view(self):
        ...
        self._ymin = None
        self._ymax = None
        # clear on-screen line immediately
        self.line.set_data([], [])
    ...
    def on_key(self, ev):
        ...
        if k == " ":
            if not self.recording:
                ...
                # stop and save (CSV + session-only PNG)
                self.recording = False
    ...
    # ---------- save (CSV + session-only PNG) ----------
    def save_csv_and_png(self):
        if not self.rows:
            print("No data to save.")
            return

        # CSV: session-only samples already in self.rows
        fname_csv = next_sequential_filename(self.label_slug, "csv")
        ...
        print(f"Saved {len(self.rows)} samples → {fpath_csv}")

        # PNG: build a small figure from the session samples written to rows
        # x-axis = session sample index (0..N-1). If you prefer time, convert timestamps to seconds.
        xs = list(range(len(self.rows)))
        ys = [r["value"] for r in self.rows]
        if xs and ys:
            # Create a one-off figure for the snapshot
            fig2, ax2 = plt.subplots(figsize=(12, 4))
            ax2.plot(xs, ys, linewidth=1.25)
            ax2.set_title(f"EMG — {self.label_human} (session)")
...
def main():
    ...
    print("Session-only plot resets on start; session-only PNG saved on stop.")
